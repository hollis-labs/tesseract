...
func TestOpen_WithQueue_UsesAdapter(t *testing.T) {
	...
	if err := q.Push(context.Background(), "test", []byte("{}"), queue.OnQueue("tesseract")); err != nil {
		t.Fatalf("Push: %v", err)
	}
	n, err := q.Size(context.Background(), "tesseract")
	if err != nil {
		t.Fatalf("Size: %v", err)
	}
	if n != 1 {
...
func TestOpen_WithQueue_WorkerProcessesJob(t *testing.T) {
	...
	if err := q.Push(context.Background(), "embed", payload, queue.OnQueue("tesseract")); err != nil {
		t.Fatalf("Push: %v", err)
	}
