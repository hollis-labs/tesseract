...
**`payload_data`** carries the entry's own fields as a JSON object — stored verbatim, never interpreted, not indexed and not searched. The contract and the limits of "verbatim" are stated once in `tesseract_skills memory` and are identical here. Note the shape: `POST /v1/event/write` takes it **flat at the top level** as `data` and `data_schema_hash`, matching how this route already takes `summary` and `body` flat, while `/v1/memory/write` nests them under `payload`. The HTTP route decodes strictly, so the wrong shape there is a `400` rather than a dropped field. **MCP refuses a wrong NAME the same way** since 2026-09-12: `data` instead of `payload_data` is a `validation_error` naming `payload_data`, not a successful write with the object missing. Over MCP it is `payload_data` on all three tools, and on read every domain returns it at `payload.data`.
