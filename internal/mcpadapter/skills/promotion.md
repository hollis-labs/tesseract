...
2. **Approve** - `context_promote` with `stage: "approve"` (scope: `promote.approve`). Moves a `pending` request to `approved`. Required: `request_id`. Optional: `notes`, `actor` (default `agent` — CW-20260910-0046 flipped it from `user` because an unattributed approval is far more often an agent than a person, and claiming `user` should take a deliberate act). Writes an approval record to `user/promotions` and appends an updated request revision.
