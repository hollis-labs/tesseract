...
- `EmitNamespaceRegister`, `EmitNamespaceUpdate`
...
- `EmitRevision(ctx, domain, op, ...)` — the shared memory/knowledge/event revision path. It composes the event type as `{domain}.{op}` from a closed set of four ops, so a domain added to the registry emits correctly without `contextstore` learning its name
