...
func (a *Adapter) registerBulkTools(s *server.MCPServer) {
	s.AddTool(mcp.NewTool("context_bulk_ingest",
		mcp.WithDescription("Batch write multiple records with optional type validation and embedding. Accepts a JSON array of items. Each item is validated, written, and optionally embedded. Returns per-item results."),
	...
	s.AddTool(mcp.NewTool("context_chunked_ingest",
		mcp.WithDescription("Ingest a large document by chunking it into multiple records, each auto-embedded. Useful for RAG pipelines with documents exceeding embedding context windows."),
