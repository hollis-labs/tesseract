...
type PgVectorConfig struct {
	// ConnString is the PostgreSQL connection string (e.g. "postgres://user:pass@host:5432/cortex").
	ConnString string
...
func (p *PgVectorIndex) ensureSchema(ctx context.Context) error {
	stmts := []string{
		...
		fmt.Sprintf(`CREATE TABLE IF NOT EXISTS cortex_vectors (
		...
		`CREATE INDEX IF NOT EXISTS idx_cortex_vectors_vec ON cortex_vectors USING ivfflat (vector vector_cosine_ops) WITH (lists = 100)`,
		`CREATE INDEX IF NOT EXISTS idx_cortex_vectors_ns ON cortex_vectors (namespace)`,
		`CREATE INDEX IF NOT EXISTS idx_cortex_vectors_type ON cortex_vectors (record_type)`,
	}
...
func (p *PgVectorIndex) Upsert(ctx context.Context, entry VectorEntry) error {
	...
	_, err := p.db.ExecContext(ctx, `
		INSERT INTO cortex_vectors (record_id, namespace, key_name, record_type, model, dimensions, vector)
...
func (p *PgVectorIndex) Delete(ctx context.Context, recordID, model string) error {
	_, err := p.db.ExecContext(ctx, `DELETE FROM cortex_vectors WHERE record_id = $1 AND model = $2`, recordID, model)
	return err
}
