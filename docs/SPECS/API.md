...
and `body` flat. Every route decodes strictly, so the wrong shape is a `400` rather than a silently
dropped field. Responses are uniform: `payload.data` on all three.
