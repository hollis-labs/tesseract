...
function PolicyList() {
  ...
  return (
    <div>
      <div style={{ marginBottom: '0.5rem', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
        <button className="hud-button-ghost" onClick={() => { refresh(); loadPolicies(); }} disabled={loading || loadingPolicies}>
          ...
          return (
            <div key={ns} style={{ borderBottom: '1px solid rgba(var(--border) / 0.5)' }}>
              ...
              >
                ...
                {!policy && (
                  <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: 'rgb(var(--muted))' }}>
                    no policy
                  </span>
                )}
              </div>
                ...
                <div style={{ padding: '0 0.75rem 0.75rem' }}>
                  {policy ? (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                      <div>
                        <div className="hud-label" style={{ marginBottom: '0.25rem' }}>Owner</div>
                        <div style={{ fontSize: '0.8rem' }}>{policy.owner_type}:{policy.owner_id}</div>
                      </div>
                      <div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'rgb(var(--muted))' }}>
                      No policy registered for this namespace. Use "Register Namespace" to create one.
                    </div>
                  )}
                </div>
              )}
...
function RegisterForm({ onRegistered }: RegisterFormProps) {
    ...
    try {
      await registerNamespace(
        ...
        ownerId.trim(),
        {
          ...
          max_revisions: maxRevisions.trim() ? parseInt(maxRevisions) : undefined,
          max_bytes_per_key: maxBytesPerKey.trim() ? parseInt(maxBytesPerKey) : undefined,
          allowed_ops: allowedOps.trim() ? allowedOps.split(',').map(s => s.trim()).filter(Boolean) : undefined,
        },
      );
  ...
  return (
    <div className="hud-panel" style={{ padding: '1rem', maxWidth: 700 }}>
      ...
      <div className="hud-label" style={{ marginBottom: '0.5rem', color: 'rgb(var(--primary))' }}>Namespace</div>
      <div className="form-grid">
        <div className="form-field form-field-full">
          <label className="hud-label">Namespace Pattern *</label>
          <input className="hud-input" placeholder="app/my-project/*" value={namespace} onChange={e => setNamespace(e.target.value)} style={{ width: '100%' }} />
        </div>
      </div>

      <div className="hud-label" style={{ marginBottom: '0.5rem', marginTop: '0.75rem', color: 'rgb(var(--primary))' }}>Owner</div>
      <div className="form-grid">
        <div className="form-field">
          <label className="hud-label">Owner Type *</label>
          <select className="hud-input" value={ownerType} onChange={e => setOwnerType(e.target.value)} style={{ width: '100%' }}>
        ...
        <div className="form-field">
          ...
          <input className="hud-input" placeholder="jane" value={ownerId} onChange={e => setOwnerId(e.target.value)} style={{ width: '100%' }} />
        </div>
      </div>

      <div className="hud-label" style={{ marginBottom: '0.5rem', marginTop: '0.75rem', color: 'rgb(var(--primary))' }}>Policy</div>
      <div className="form-grid">
        <div className="form-field">
          <label className="hud-label">Tier</label>
          <select className="hud-input" value={tier} onChange={e => setTier(e.target.value)} style={{ width: '100%' }}>
        ...
        <div className="form-field">
          ...
          <input className="hud-input" placeholder="720h (30 days)" value={retention} onChange={e => setRetention(e.target.value)} style={{ width: '100%' }} />
        </div>
        <div className="form-field">
          <label className="hud-label">Max Revisions</label>
          <input className="hud-input" type="number" placeholder="100" value={maxRevisions} onChange={e => setMaxRevisions(e.target.value)} style={{ width: '100%' }} />
        </div>
        <div className="form-field">
          <label className="hud-label">Max Bytes/Key</label>
          <input className="hud-input" type="number" placeholder="1048576" value={maxBytesPerKey} onChange={e => setMaxBytesPerKey(e.target.value)} style={{ width: '100%' }} />
        </div>
        <div className="form-field form-field-full">
          <label className="hud-label">Allowed Ops <span style={{ color: 'rgb(var(--muted))' }}>(comma-separated)</span></label>
          <input className="hud-input" placeholder="read, write, promote" value={allowedOps} onChange={e => setAllowedOps(e.target.value)} style={{ width: '100%' }} />
        </div>
      </div>
