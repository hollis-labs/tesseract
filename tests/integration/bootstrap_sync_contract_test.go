...
type bootstrapSyncGolden struct {
...
func TestBootstrapSyncScriptReportAndApply(t *testing.T) {
	golden := loadBootstrapSyncGolden(t)
	root := t.TempDir()
	...
	if err != nil {
		t.Fatalf("bootstrap-sync report failed: %v\noutput:\n%s", err, string(reportOut))
	}
	...
	if err != nil {
		t.Fatalf("bootstrap-sync apply failed: %v\noutput:\n%s", err, string(applyOut))
	}
...
func loadBootstrapSyncGolden(t *testing.T) bootstrapSyncGolden {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("fixtures", "bootstrap_sync_contract_golden.json"))
	if err != nil {
		t.Fatalf("read bootstrap sync golden: %v", err)
	}
	var out bootstrapSyncGolden
	if err := json.Unmarshal(data, &out); err != nil {
		t.Fatalf("unmarshal bootstrap sync golden: %v", err)
	}
	return out
}
